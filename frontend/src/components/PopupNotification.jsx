  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm">
      <div className={`w-full max-w-md mx-auto transform transition-all duration-300 scale-100 animate-in zoom-in-95 fade-in-0`}>
        <div className={`${getColors()} border-2 rounded-2xl shadow-2xl p-6 backdrop-blur-md`}>
          <div className="flex items-start space-x-4">
            ...
            <button
              onClick={onClose}
              className="flex-shrink-0 ml-4 p-1 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
